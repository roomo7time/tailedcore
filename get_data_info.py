...
def analyze_gap(extracted_path: str, data_name: str, config_name: str):
    ...
    num_samples_per_class = dict(Counter(class_names))
    noise_count_per_class = defaultdict(int)
    for class_name in class_names:
        noise_count_per_class[class_name]
    for class_name, is_noise in zip(class_names, labels):
        if is_noise:  # Check if the current sample is marked as noise
            # Increment the noise count for the class, initializing at 0 if not already present
            noise_count_per_class[class_name] = noise_count_per_class.get(class_name, 0) + 1

    noise_count_per_class = dict(noise_count_per_class)
    save_log_dir = os.path.join("./logs", f"{data_name}_{config_name}")

    save_data_info_path = os.path.join(save_log_dir, "num_samples_per_class.csv")
    utils.save_dicts_to_csv([num_samples_per_class, noise_count_per_class], save_data_info_path)
