...
def get_coreset_model(
    ...
    **kwargs,
    ...
    elif model_config.coreset_model_name == "augtailedpatch":
        return AugTailedPatch(
            feature_embedder,
            device,
            imagesize,
            coreset_ratio=coreset_ratio,
            ...
            save_dir_path=save_dir_path,
            brute=brute,
            augment=getattr(model_config, 'augment', False),
        )
    elif model_config.coreset_model_name == "adatailedpatch":
        return AdaTailedPatch(
            feature_embedder,
            device,
            imagesize,
            coreset_ratio=coreset_ratio,
...
class AugTailedPatch(BaseCore):
    ...
    def __init__(
        ...
        super(AugTailedPatch, self).__init__()
...
class AdaTailedPatch(BaseCore):
    ...
    def __init__(
        ...
        super(AdaTailedPatch, self).__init__()
    ...
    def _get_coreset(self, trainloader: DataLoader) -> torch.Tensor:
        ...
        tail_indices = self._sample_tail_indices(embeddings)
        
        self._get_tail_augmented_features(trainloader, tail_indices)
    ...
    #                 # Select only the features corresponding to relevant indices
    #                 relevant_features = _features[[i - batch_idx * batch_size for i in relevant_indices]]
    #                 features.append(relevant_features)
    ...
    def _sample_tail_indices(self, embeddings):

        h, w = self.feature_map_shape[0], self.feature_map_shape[1]

        _, tail_embedding_indices = self.tail_sampler.run(embeddings)

        return tail_embedding_indices
